...
module("Offer Model", function (hooks) {
  setupTest(hooks);

  test("check attributes", function (assert) {
    assert.expect(22);
    var model = run(() =>
      this.owner.lookup("service:store").createRecord("offer")
    );
    ...
    var estimatedSize =
      Object.keys(model.toJSON()).indexOf("estimatedSize") > -1;
  ...
  test("Relationships with other models", function (assert) {
    ...
    var offer = this.owner.lookup("service:store").modelFor("offer");
    var relationshipsWithGgvTransport = get(offer, "relationshipsByName").get(
      "gogovanTransport"
    );
    ...
    var relationshipsWithItem = get(offer, "relationshipsByName").get("items");
    ...
    assert.equal(
      relationshipsWithCrossroadsTransport.key,
      "crossroadsTransport"
    );
