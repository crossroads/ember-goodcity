...
module("GogovanOrder Model", function (hooks) {
  setupTest(hooks);

  test("check attributes", function (assert) {
    assert.expect(26);
    var model = run(() =>
      this.owner.lookup("service:store").createRecord("gogovan_order")
    );
    ...
    var gogovanOptionId =
      Object.keys(model.toJSON()).indexOf("gogovanOptionId") > -1;
    ...
    var needEnglishFee =
      Object.keys(model.toJSON()).indexOf("needEnglishFee") > -1;
    ...
    var driverLicense =
      Object.keys(model.toJSON()).indexOf("driverLicense") > -1;
  ...
  test("Relationships with other models", function (assert) {
    ...
    var gogovan_order = this.owner
      .lookup("service:store")
      .modelFor("gogovan_order");
    var relationshipsWithDelivery = get(
      gogovan_order,
      "relationshipsByName"
    ).get("delivery");
