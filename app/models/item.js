...
export default DS.Model.extend({
  ...
  removePrivateMessage: observer(
    "messages.[]",
    "messages.@each.isPrivate",
    function () {
  ...
  canUpdated: computed("hasReceivedPackages", "offer.state", function () {
    return !(
      this.get("hasReceivedPackages") ||
      this.get("offer.isFinished") ||
      this.get("offer.state") === "receiving"
    );
  }),
  ...
  isSubmitted: computed("state", "offer.state", function () {
    return (
      this.get("state") === "submitted" &&
      this.get("offer.state") === "submitted"
    );
  }),
  ...
  isUnderReview: computed("state", "offer.state", function () {
    return (
      this.get("state") === "submitted" &&
      this.get("offer.state") === "under_review"
    );
  }),
  ...
  displayImage: computed("images.@each.favourite", function () {
    return (
      ...
      this.get("images").sortBy("id").get("firstObject") ||
      null
    );
  }),
  ...
  displayImageUrl: computed(
    "displayImage",
    "displayImage.thumbImageUrl",
    function () {
      return (
        this.get("displayImage.thumbImageUrl") ||
