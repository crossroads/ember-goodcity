...
export default Ember.Controller.extend({
  ...
  images: Ember.computed("item.images.[]", function() {
    ...
    return (this.get("item.images") || Ember.A())
      .filterBy("itemId")
      .toArray()
      .sort(function(a, b) {
  ...
  favouriteImage: Ember.computed(
    "item.images.@each.favourite",
    "package.favouriteImage",
    function() {
  ...
  initPreviewImage: Ember.on(
    "init",
    Ember.observer("package", "item", "item.images.[]", function() {
    ...
    if (offer.get("itemCount") === 1) {
      ...
      if (delivery) {
        this.get("messageBox").confirm(
          this.locale("edit_images.cancelling_item_will_cancel_offer"),
          () => {
  ...
  cannotRemoveImageAlert: function() {
    this.get("messageBox").alert(
      this.locale("edit_images.cant_delete_last_image")
    );
  },
  ...
  actions: {
    ...
    triggerUpload() {
      ...
      if (config.cordova.enabled) {
        ...
        if (navigator.userAgent.match(/iemobile/i)) {
          ...
          Ember.$("#photo-list input[type='file']")
            .click()
            .click();
        } else {
    ...
    uploadSuccess(e, data) {
      ...
      if (!item || this.get("item.isOffer")) {
        var defaultDonorCondition = this.get("store")
          .peekAll("donorCondition")
          .sortBy("id")
          .get("firstObject");
