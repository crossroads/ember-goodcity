...
export default Controller.extend({
  ...
  districtName: computed("model.districtId", function () {
    var district = this.store.peekRecord(
      "district",
      this.get("model.districtId")
    );
  ...
  actions: {
    confirmOrder() {
      ...
      var scheduleProperties = {
        scheduledAt: orderDetails.get("pickupTime"),
        slotName: orderDetails.get("slot"),
      };

      var delivery = controller.store.peekRecord(
        "delivery",
        controller.get("deliveryController.model.id")
      );
      var offer = delivery.get("offer");

      orderDetails.setProperties({
        name: name,
        mobile: mobile,
        offerId: offer.get("id"),
      });

      contactProperties.addressAttributes = {
        addressType: "collection",
        districtId: orderDetails.get("districtId"),
      };
